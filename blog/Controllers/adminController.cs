...
namespace blog.Controllers
{
    ...
    {
    public BlogDataEntities4 db = new BlogDataEntities4();
        // GET: admin
        // HÀM ACCOUNT
        [HttpGet]
        [NonAction]
       ...
       //HÀM STATISTICAL
        ...
        public ActionResult Post(string searchkey="", int page=1)
        {
            ...
            int pageSize = 5;
            List<Blog> ListBlog = new List<Blog>();
            ListBlog = db.Blogs.Where(x => x.Title.Contains(searchkey)).Where(item => db.Actions.Where(i => i.BlogId == item.BlogId &&
            i.Report == true).ToList().Count() >0).OrderByDescending(s => s.TotalView).ToList();
    ...
    public ActionResult AddCategories([Bind(Include ="CategoryId,Name")]Category category)
        {
